...
from helpers.detectron2 import CocoTrainer, LossEvalHook, dt2predictions_to_list
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="This script trains a predictive models."
    )
    ...
    if MODEL_ZOO_CHECKPOINT_URL == None:
        logger.critical(
            'A model zoo checkpoint URL ("model_zoo_checkpoint_url") must be provided'
        )
        sys.exit(1)
    ...
    # Create a timestamped log directory for this training run
    # This allows multiple runs to be compared in TensorBoard
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    LOG_SUBDIR = os.path.join(cfg["log_subfolder"], f"run_{timestamp}")
    logger.info(f"Logs will be saved to: {LOG_SUBDIR}")
    ...
    for dataset in registered_datasets:
        for d in DatasetCatalog.get(dataset)[
            0 : min(len(DatasetCatalog.get(dataset)), 4)
        ]:
            ...
            visualizer = Visualizer(
                img[:, :, ::-1], metadata=MetadataCatalog.get(dataset), scale=1.0
            )

            vis = visualizer.draw_dataset_dict(d)
            cv2.imwrite(
                os.path.join(SAMPLE_TAGGED_IMG_SUBDIR, output_filename),
                vis.get_image()[:, :, ::-1],
            )
            written_files.append(
                os.path.join(
                    WORKING_DIR, os.path.join(SAMPLE_TAGGED_IMG_SUBDIR, output_filename)
                )
            )
    ...
    for d in DatasetCatalog.get("tst_dataset")[
        0 : min(len(DatasetCatalog.get("tst_dataset")), 10)
    ]:
        ...
        v = Visualizer(
            im[
                :, :, ::-1
            ],  # [:, :, ::-1] is for RGB -> BGR conversion, cf. https://stackoverflow.com/questions/14556545/why-opencv-using-bgr-colour-space-instead-of-rgb
    ...
    toc = time.time()
    logger.info(
        f"Nothing left to be done: exiting. Elapsed time: {(toc - tic):.2f} seconds"
    )
